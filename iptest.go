...
var (
	...
	sampleSize   = flag.Int("sample", 1000, "从每个CIDR网段中随机测试的IP数量，设为0测试所有IP")               // 从每个CIDR网段中随机测试的IP数量
...
type AirportCode struct {
	...
	CIDRs   []string `json:"cidrs,omitempty"` // 该机场码对应的CIDR网段
}
...
var airportCodes = map[string]AirportCode{
	...
	// 其他地区...
	// 注意：由于Cloudflare的IP分配是动态的，上面的CIDR只是示例
	// 实际使用时需要根据Cloudflare的官方IP分配来更新
}
...
func getIPRangesByAirportCodes(codes []string) ([]string, error) {
	fmt.Printf("正在为机场码 %v 获取对应的IP网段...\n", codes)
	
	var allRanges []string
	
	for _, code := range codes {
		if airport, exists := airportCodes[code]; exists {
			fmt.Printf("机场 %s (%s) 有 %d 个CIDR网段\n", code, airport.Name, len(airport.CIDRs))
			allRanges = append(allRanges, airport.CIDRs...)
		} else {
			fmt.Printf("警告: 未知的机场码 %s\n", code)
		}
	}
	
	if len(allRanges) == 0 {
		fmt.Println("没有找到指定机场码的IP网段，使用所有Cloudflare IP范围")
		return getCloudflareIPRanges()
	}
	
	// 去重
	uniqueRanges := make(map[string]bool)
	for _, r := range allRanges {
		uniqueRanges[r] = true
	}
	
	result := make([]string, 0, len(uniqueRanges))
	for r := range uniqueRanges {
		result = append(result, r)
	}
	
	fmt.Printf("为机场码 %v 找到 %d 个唯一的CIDR网段\n", codes, len(result))
	return result, nil
}
...
func sampleIPsFromCIDR(cidr string, sampleSize int) ([]string, error) {
	...
	if sampleSize == 0 || totalIPs <= sampleSize {
		// 如果采样数量为0或者IP数量小于等于采样数量，测试所有IP
		fmt.Printf("网段 %s 包含 %d 个IP地址，测试所有IP\n", cidr, totalIPs)
		...
		// 如果指定了采样数量且IP数量大于采样数量，使用高效的随机采样算法
		fmt.Printf("网段 %s 包含 %d 个IP地址，随机采样 %d 个IP进行测试\n", cidr, totalIPs, sampleSize)
...
func main() {
	...
	// 如果指定了机场码，直接从对应的CIDR网段扩展IP
	if *region != "" {
		fmt.Printf("正在根据机场码 %v 获取对应的IP网段...\n", selectedCodes)
		ipRanges, err := getIPRangesByAirportCodes(selectedCodes)
		if err != nil {
			fmt.Printf("获取机场码IP网段失败: %v\n", err)
